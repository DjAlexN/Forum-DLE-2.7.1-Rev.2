...
function doAddPost()
{
    ...
    if (post_text == '')
    {
        DLEalert ('Prosze wypelnic wszystkie pola', 'Wystapil blad');
        return false;
    }
...
function postDelete(url) {
    DLEconfirm('Czy na pewno chcesz usunac te wiadomosc?', dle_confirm, function () { document.location = url; } );
};
...
function topicDelete(url) {
    DLEconfirm('Czy na pewno chcesz usunac ten temat?', dle_confirm, function () { document.location = url; } );
};

function rowDelete(url) {
    DLEconfirm('Czy na pewno chcesz usunac ten zapis?', dle_confirm, function () { document.location = url; } );
};
...
if (moderation)
{
    menu[0]='<a href="' + forum_url + '&act=moderation&code=calc&tid=' + tid + '">Do ostatniej strony</a>';
}

menu[1]='<a href="' + forum_url + '&act=subscription&code=add&tid=' + tid + '">Subskrybuj ten temat</a>';

menu[2]='<a href="' + forum_url + '&act=_topic&code=forward&tid=' + tid + '">Wyslij do znajomego</a>';

menu[3]='<a href="' + forum_url + '&act=_topic&code=print&tid=' + tid + '">Wersja do druku</a>';
...
menu[0]='<a href="' + forum_url + '&showforum=' + fid + '&code=hidden">Zobacz wszystkie ukryte watki</a>';

menu[1]='<a href="' + forum_url + '&act=getforum&code=calc&fid=' + fid + '">Konwertuj wszystkie fora</a>';

}

menu[3]='<a href="' + forum_url + '&showforum=' + fid + '&code=today">Aktywne tematy</a>';

menu[4]='<a href="' + forum_url + '&showforum=' + fid + '&code=noreply">Tematy bez odpowiedzi</a>';
...
var menu=new Array();
	...
	menu[2]='<a href="' + forum_url + '&act=getforum&code=user&n=' + url + '">Znajdz swoje tematy</a>';
    menu[3]='<a href="' + forum_url + '&act=posts&user=' + url + '">Wyszukaj wiadomosci uzytkownika</a>';
...
function FWarn(action, user_id, post_id, forum_id)
{
    ...
    $('body').append("<div id='warn-form' title='Ostrzezenia' style='display:none'></div>");
...
{
	DLEprompt('Podaj przyczyne zmiany reputacji:', '', 'Reputacja', function(response)
    {
        if (response)
        {
            ShowLoading();
...
{
	DLEprompt('', '', 'Idz do strony', function(page)
	{
		if (pages_count >= page)
		{
...
function PostLink(link)
{
    ...
    DLEalert(message, 'Skopiuj bezposredni link:');
};
...
function uploadsform(open_url)
{
    ...
    $("body").append("<div id='uploads-form' title='Pobierz pliki' style='display:none'></div>");
    ...
    $.post(forum_ajax + 'uploads.form.php', {open_url: open_url}, function(data)
    {
        $('#uploads-form').dialog({
            autoOpen: true, width: 470,
            buttons: {
                "Zamknij to okno": function() { $(this).dialog("close"); $("#uploads-form").remove(); }
            }
        });
